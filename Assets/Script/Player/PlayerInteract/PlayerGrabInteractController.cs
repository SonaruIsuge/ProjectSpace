...
public class PlayerGrabInteractController : IPlayerInteract
{
    ...
    private async Task PushPlayer()
    {
        ...
        while (!getItem)
        {
            ...
            ClawTransform.LookAt(clawPos - ClawWorldOriginPos);
            ClawTransform.position = Vector3.MoveTowards(clawPos, currentItemPoint, targetPlayer.ClawSpeed * Time.deltaTime);
            targetPlayer.transform.position = Vector3.MoveTowards(playerPos, currentItemPoint + clawPlayerWorldDiff, targetPlayer.ClawSpeed * Time.deltaTime);
